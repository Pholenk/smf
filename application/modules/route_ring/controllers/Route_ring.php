...
class Route_ring extends MX_Controller
{
	...
	public function browse()
	{
		if ($this->is_login)
		{
			if ($this->auth->privileges_read('ring_browse'))
			{
	...
	public function read($id)
	{
		if ($this->is_login)
		{
			if ($this->auth->privileges_read('ring_edit') && !empty($id))
			{
	...
	public function add()
	{
		if ($this->is_login)
		{
			if ($this->auth->privileges_read('ring_add'))
			{
	...
	public function delete($id)
	{
		if ($this->is_login)
		{
			if ($this->auth->privileges_read('ring_add') && !empty($id))
			{
				$this->Route_ringModel->delete($id) ? redirect(base_url('/route_ring')) : redirect(base_url());
			}
			else
			{
				$this->_show_interface('Unauthorize', '');
	 ...
	 */
	private function _show_interface($page, $data)
	{
		if(!empty($page && $data))
		{
			$this->load->view('head');
