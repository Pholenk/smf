...
class Supplier_product extends MX_Controller
{
	...
	public function browse()
	{
		if ($this->is_login)
		{
			$acces = $this->auth->privileges_read('supplier_prod_browse');
			if ($acces)
			{
	...
	public function read($id)
	{
		if ($this->is_login)
		{
			$acces = $this->auth->privileges_read('supplier_prod_edit');
			if ($acces)
			{
				if (substr($id, 0, 2) !== 'PR')
				{
	...
	public function add()
	{
		if ($this->is_login)
		{
			$acces = $this->auth->privileges_read('supplier_prod_add');
			if ($acces && !empty($this->input->post('supplier')))
			{
				...
				($this->Supplier_productModel->add($supplier_prod_data) ? redirect(base_url('/supplier_product')) : redirect(base_url()));
			}
			elseif ($acces && empty($this->input->post('supplier')))
			{
				$supplier_data = $this->Supplier_productModel->supplier_browse();
				echo "
				<div class='modal-header'>
